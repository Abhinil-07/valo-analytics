...
map_agg_df = match_df.groupBy("map_name").agg(
    ...
    F.sum(F.when(F.col("starting_side") == "Attack", F.coalesce(F.col("fh_rounds_won"), F.lit(0)))).alias("attack_start_fh_rounds_won"),
    F.sum(F.when(F.col("starting_side") == "Attack", F.coalesce(F.col("fh_rounds_played"), F.lit(0)))).alias("attack_start_fh_rounds_played"),
    ...
    F.sum(F.when(F.col("starting_side") == "Defense", F.coalesce(F.col("fh_rounds_won"), F.lit(0)))).alias("defense_start_fh_rounds_won"),
    F.sum(F.when(F.col("starting_side") == "Defense", F.coalesce(F.col("fh_rounds_played"), F.lit(0)))).alias("defense_start_fh_rounds_played"),
    # Team combat & pace
...
final_gold_map_df = dim_map_df.join(map_agg_df, on="map_name", how="inner").select(
    ...
    F.col("matches_lost"),
    F.col("matches_drawn"),
    F.round(F.col("matches_won") * 100.0 / F.col("matches_played"), 2).alias("map_win_pct"),
    F.round(F.col("matches_played") * 100.0 / F.lit(max(total_matches_all_maps, 1)), 2).alias("play_frequency_pct"),
    F.col("rounds_played"),
    F.col("rounds_won"),
    F.col("rounds_lost"),
    ...
    F.col("attack_start_matches"),
    F.col("attack_start_wins"),
    F.round(F.col("attack_start_wins") * 100.0 / F.when(F.col("attack_start_matches") == 0, 1).otherwise(F.col("attack_start_matches")), 2).alias("attack_start_win_pct"),
    F.round(F.col("attack_start_fh_rounds_won") * 100.0 / F.when(F.col("attack_start_fh_rounds_played") == 0, 1).otherwise(F.col("attack_start_fh_rounds_played")), 2).alias("attack_start_first_half_round_win_pct"),
    F.col("defense_start_matches"),
    F.col("defense_start_wins"),
    F.round(F.col("defense_start_wins") * 100.0 / F.when(F.col("defense_start_matches") == 0, 1).otherwise(F.col("defense_start_matches")), 2).alias("defense_start_win_pct"),
    F.round(F.col("defense_start_fh_rounds_won") * 100.0 / F.when(F.col("defense_start_fh_rounds_played") == 0, 1).otherwise(F.col("defense_start_fh_rounds_played")), 2).alias("defense_start_first_half_round_win_pct"),
